...
const uint8_t Addr_Loudness = 0b00000001 //不启用响度，CS1和CS2都接地
...
void setup() {
  ...
  //初始化SPI
  SPI.begin();
  SPI.beginTransaction(SPISettings(1000000, LSBFIRST, SPI_MODE0));//根据数据手册，芯片工作频率为0~1MHz，寄存器数据写入顺序为LSB
...
void loop() 
{ 
    for (Volume = 0; Volume <=89; Volume++) //控制音量衰减由小到大（音量从大到小变化），衰减控制数值从0（0dB）增加到89（-89dB）
    {
       TC9459Write(Volume, Volume, LOUDNESS);
       Serial.print("Volume=");
       Serial.println(Volume);
       delay(10);
    }

    for (Volume = 89; Volume >=0; Volume--) //控制音量衰减由大到小（音量从小到大变化），衰减控制数值从89（-89dB）减小到0（0dB）
    {
       TC9459Write(Volume, Volume, LOUDNESS);
       Serial.print("Volume=");
       Serial.println(Volume);
...
//TC9459F写入函数
void TC9459Write(uint8_t leftAttenuation, uint8_t rightAttenu)
{
